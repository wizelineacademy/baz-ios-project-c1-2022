...
class MovieAPI {
        ...
        for result in results {
            if let id = result.object(forKey: "id") as? Int,
               ...
               let poster_path = result.object(forKey: "poster_path") as? String, let strDetail = result.object(forKey: "overview") as? String {
                movies.append(Movie(id: id, title: title, descriptionMovie: strDetail, poster_path: poster_path))
            }
        }
        return movies
    }
            ...
            if let data = data {
                do {
                    ...
                    if let dataInfo = json as? [String : Any], let dataAll = dataInfo["results"] as? [[String: Any]]{
                        ...
                        for item in dataAll {
                            lstMovies.append(Movie(id: item["id"] as? Int ?? 0, title: item["name"] as? String ?? "", descriptionMovie: item["overview"] as? String ?? "", poster_path: item["poster_path"] as? String ?? ""))
                        }
                    }
    ...
    func serializationJson(jsonDic: Any) -> [Movie] {
        ...
        if let dataInfo = jsonDic as? [String : Any], let dataAll = dataInfo["results"] as? [[String: Any]]{
            for item in dataAll {
                lstMovies.append(Movie(id: item["id"] as? Int ?? 0, title: item["title"] as? String ?? "", descriptionMovie: item["overview"] as? String ?? "", poster_path: item["poster_path"] as? String ?? ""))
            }
        }
        return lstMovies
    }
